...
class ColorMix(BaseEffect):
    ...
    def __init__(self, resolution: tuple[int, int], dimensions: tuple[int, int], rgbw: bool, settings_manager=None) -> None:
        ...
        self.inputs['seed'] = Fader(0) # 1 channel - random seed for the noise pattern
    ...
    def _generate_noise(self):
        ...
        # Settings that determine the noise pattern
        current_settings = (mix_mode, category, noise_scale, seed,
        ...
        if self._noise_pattern is not None and self._last_noise_settings == current_settings:
        ...
        if mix_mode == 'Image':
            ...
            # Use only one channel (as it's grayscale) and transpose to (W, H)
            self._noise_pattern = frame[:, :, 0].T.astype(float) / 255.0
        else:
            # Determine grid size for noise generation (min 2, max full resolution)
            min_grid = 2
            max_grid = (max(self.res_x, self.res_y))/2
            grid_size = int(min_grid + (noise_scale / 255.0) * (max_grid - min_grid))
            grid_size = max(min_grid, grid_size)

            # Use a deterministic seed from category and seed fader
            rng = np.random.RandomState(seed=int(category) * 1000 + seed)
                ...
                with ui.column().classes('w-full px-4 pb-4'):
                    ...
                    with ui.row().classes('w-full q-gutter-sm'):
                        with ui.column():
                            ...
                            self.inputs['blend'].on_ui_input = on_param_change
                        
                        with ui.column():
                            ui.label('Noise Scale / Frequency').classes('text-xs text-gray-500 mt-2')
                            self.inputs['noise_scale'].ui_input()
                            self.inputs['noise_scale'].on_ui_input = on_param_change

                        with ui.column():
                            ui.label('Seed').classes('text-xs text-gray-500 mt-2')
                            self.inputs['seed'].ui_input()
                            self.inputs['seed'].on_ui_input = on_param_change
